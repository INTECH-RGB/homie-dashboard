    ...
    async getStatDevice(id, interval){
        switch(interval){
            ...
            var result = await this.knex.select(this.knex.raw("DATE(property_history.date/1000, 'unixepoch') as day"),
                                                this.knex.raw("(strftime('%H', datetime(property_history.date / 1000, 'unixepoch')) + 1) as hour"),
                                                this.knex.raw("(strftime('%M', datetime(property_history.date / 1000, 'unixepoch'))) as minute"),
                                            ...
                                            .where('hour', actualDateSecond.getHours())
                                            .groupBy("day", "hour", "minute", "property_id")
                                            .orderBy("day", "hour", "minute", 'ASC')
            return result     
            break
